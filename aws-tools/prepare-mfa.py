...
prm_credfile = 'C:/users/Anatolii_Maslov/.aws/credentials'  # filename where profile creds are stored
...
def test_credentials(tempprofile_name: str):
    session = boto3.session.Session(profile_name=tempprofile_name)
    client = session.client('sts')
